...
│   └── cicids2017.yaml        CICIDS2017, the second dataset used by Ding et al.
├── src/
...
├── scripts/
│   ├── download_data.py       downloads and verifies UNSW-NB15
│   ├── paper_reference.py     rebuilds outputs/paper_reference from the paper's printed values
│   └── make_demo_data.py      placeholder data for smoke tests (not real traffic)
├── outputs/
│   ├── sample_run/            a real run on UNSW-NB15 (committed)
│   └── paper_reference/       the paper's own numbers, for comparison (committed)
├── tests/test_smoke.py
...
python scripts/download_data.py                        # fetches + verifies UNSW-NB15
```

Ding et al. (TMG-GAN) evaluate on **CICIDS2017 and UNSW-NB15**; this paper uses UNSW-NB15, which is
the default here. CICIDS2017 is supported too — see [`data/README.md`](data/README.md).
...
python main.py --config configs/cicids2017.yaml  # CICIDS2017
...
Two result sets are committed to the repository:

- **[`outputs/sample_run/`](outputs/sample_run/)** — a real run on the official UNSW-NB15 training
  partition, reproducible with `python main.py --output-dir outputs/sample_run`.
- **[`outputs/paper_reference/`](outputs/paper_reference/)** — the values printed in the paper,
  transcribed for comparison (regenerate with `python scripts/paper_reference.py`). That folder's
  README also lists the inconsistencies found in Tables 1–2 and Fig. 5.

### Sample run vs the paper (five classes, seed 42, held-out test split)

| Metric | Focal + Attention (this run) | Cross-Entropy (this run) | Focal + Attention (paper, Table 1) |
|---|---|---|---|
| Accuracy | 0.9542 | 0.9431 | 0.8291 |
| Precision | 0.8435 | 0.7977 | 0.8675 |
| Recall | 0.7518 | 0.6455 | 0.8121 |
| F1-score | 0.7829 | 0.6713 | 0.8177 |
| FAR | 0.0104 | 0.0120 | not reported |

The columns are not directly comparable: the paper's Table 2 is scored on training plus synthetic
data, this run on a held-out split of real records only. What does carry over is the comparison
*within* a column — attention plus focal loss beats the cross-entropy baseline on every metric,
most clearly on recall and F1.
