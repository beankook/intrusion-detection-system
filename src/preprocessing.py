...
def load_raw(train_csv: str, test_csv: str | None = None) -> pd.DataFrame:
    """Read the UNSW-NB15 partition file(s) and return one cleaned DataFrame."""
    paths = [p for p in (train_csv, test_csv) if p and Path(p).exists()]
    if not paths:
        raise FileNotFoundError(
            f"No dataset found at '{train_csv}'. See data/README.md for download instructions, "
            "or run `python main.py --demo` for a smoke test on generated placeholder data."
        )
